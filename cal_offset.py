...
def transform_error(x, A_to_C_list, B_to_C_list):
    ...
    for A_to_C, B_to_C in zip(A_to_C_list, B_to_C_list):
        ...
        error = np.linalg.norm(predicted_A_to_C - A_to_C)
        total_error += error
...
if __name__ == "__main__":
    ...
    for i in range(5):
        ...
        # 添加噪声
        noise_rotation = Rotation.from_euler('xyz', np.random.rand(3)*2-1, degrees=True).as_matrix()
        noise_translation = (np.random.rand(3) * 0.1 - 0.05)
        
        A_to_C[:3, :3] = A_to_C[:3, :3] @ noise_rotation
    ...
    print("\n估计误差:")
    print(np.linalg.norm(true_A_to_B - estimated_A_to_B))